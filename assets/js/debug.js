...
function sanitizeValue(value, depth = 0) {
    ...
    if (value instanceof URL) return value.origin === window.location.origin ? value.pathname : value.origin;
    if (value instanceof Element) {
        const id = value.id ? `#${value.id}` : "";
        const className = typeof value.className === "string"
            ? value.className.trim().split(/\s+/).filter(Boolean).slice(0, 2).map((name) => `.${name}`).join("")
            : "";
